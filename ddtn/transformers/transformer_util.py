...
from ddtn.transformers.transformer_layers import ST_CPAB_transformer, ST_CPAB_train_transformer
...
def get_transformer_layer(transformer_name='affine'):
    ...
    lookup = {'affine': ST_Affine_transformer,
              ...
              'CPAB_train': ST_CPAB_train_transformer,
...
def get_random_theta(N, transformer_name='affine'):
    ...
    if transformer_name == 'affine':
        ...
        theta = 0.05*np.random.normal(size=(N, dim))
        # theta = np.full((N, dim), 0.000001)
    # elif transformer_name == 'CPAB_train':
